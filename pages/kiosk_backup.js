...
// Helper function to narrate an action and then execute it
// Usage: narratedAction("Go to cart", () => setScreen("cart"))
const narratedAction = (actionText, callback, language = "en") => {
  return async () => {
    if (typeof window !== "undefined" && window.speechSynthesis) {
      speak(actionText, language);
      // Small delay to let narration start before action executes
...
export default function KioskPage() {
  ...
  // When language changes, reinitialize Google Translate widget
  useEffect(() => {
    if (window.google && window.google.translate) {
      try {
        ...
        console.log("Google Translate reinit:", err);
      }
    }
  }, [language]);
  ...
  async function handleLanguageChange(langCode) {
    ...
    speak(`Language changed to ${languageNames[langCode]}`, langCode);
  }
  ...
  const DrinkDetailsPage = () => {
    ...
    const toggleTopping = (topping) => {
      const isAdding = !selectedToppings.some((t) => t.id === topping.id);
      
      if (narrationOn) {
        if (isAdding) {
          speak(`${topping.name} added. Plus $${Number(topping.price).toFixed(2)}.`, language);
        } else {
          speak(`${topping.name} removed.`, language);
        }
      }
    ...
    return (
      ...
      <select
        ...
        onChange={(e) => {
          ...
          if (narrationOn) {
            speak(`Sweetness set to ${e.target.value}`, language);
          }
        }}
        style={{
          padding: "10px",
      ...
      >
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (narrationOn) {
              speak(`Temperature set to ${e.target.value}`, language);
            }
          }}
          style={{
            padding: "10px",
          ...
          <>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (narrationOn) {
                  speak(`Ice level set to ${e.target.value}`, language);
                }
              }}
              style={{
                padding: "10px",
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (narrationOn) {
              speak(`Size set to ${e.target.value}`, language);
            }
          }}
          style={{
            padding: "10px",
        ...
        <button
          onClick={() => {
            if (narrationOn) {
              speak(`${detailsItem.name} added to cart.`, language);
            }
            finalize();
          }}
          style={{
    ...
    return (
      ...
      >
        ...
        <button
          ...
          onPointerUp={() => {
            if (narrationOn) {
              speak("Proceeding to payment.", language);
            }
            setScreen("payment");
          }}
        ...
        <button
          ...
          onPointerUp={() => {
            if (narrationOn) {
              speak("Returning to menu.", language);
            }
            setScreen("menu");
          }}
          style={{
  ...
  const PaymentScreen = ({ accessibilityMode, narrationOn, language }) => {
    ...
    if (!accessibilityMode) {
      return (
        ...
        >
          ...
          <h2 style={{ fontSize: "36px", marginTop: "60px" }}>Payment</h2>
          ...
          {paymentMethods.map((method) => (
            <button
              ...
              onClick={() => {
                if (narrationOn) {
                  speak(`Processing payment with ${method}.`, language);
                }
                handlePayment();
              }}
              style={{
          ...
          <button
            onClick={() => {
              if (narrationOn) {
                speak("Returning to cart.", language);
              }
              setScreen("cart");
            }}
            style={{
    ...
    return (
      ...
      >
        ...
        <h2 style={{ fontSize: "44px", marginTop: "80px" }}>Payment</h2>
  ...
  const SuccessScreen = ({
    ...
    return (
      ...
      >
        ...
        <h1
          style={{ fontSize: "48px", marginBottom: "10px", marginTop: "60px" }}
        >
          Payment Successful!
        </h1>
        <p
        ...
        <button
          onClick={() => {
            if (narrationOn) {
              speak("Thank you for your order!", language);
            }
            setCart([]);
            setLastOrderId(null);
